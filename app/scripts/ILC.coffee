...
window.ILC = 
	...
	initialize: (opts)->
		...
		ILC.addPolygons('charlotte', opts.limit)
		ILC.loadData()
	...
	addPolygons: (dataset, limit) ->
		...
		if Settings.useLocalData
			uri = routes.industrial_polygons(dataset)
			url_industrial = uri + limit_str
		else
			if limit <= 500
				url_industrial = 'json-data/development-polygons-500.json'
				url_converted = 'json-data/converted-polygons-500.json'
			else
				url_industrial = 'json-data/development-polygons-all.json'
				url_converted = 'json-data/converted-polygons-all.json'
	...
	loadData: ->
		...
		res = HTTP.blocking 'GET', 'json-data/development-naics-trends.json'
		res.success (data) =>
			@naics_trends = data.naics_trends

		res = HTTP.blocking 'GET', 'json-data/naics-list.json'
		res.success (data) =>
			@naics_list = data.naics_list

		res = HTTP.blocking 'GET', 'json-data/brownfields.geojson'
		res.success (data) =>
